...
class Build_GUI(object):
  ...
  def center_window(self, all_skype_profiles, width=220, height=0):
...
class Action(Build_GUI):
  ...
  def add_restricted_site(self):
    ...
    key = wreg.CreateKey(wreg.HKEY_CURRENT_USER, "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Internet Settings\\ZoneMap\\Domains\\{0}".format(skype_domain,skype_apps_domain))
    wreg.CloseKey(key)
    key = wreg.CreateKey(wreg.HKEY_CURRENT_USER, "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Internet Settings\\ZoneMap\\Domains\\{0}\\{1}".format(skype_domain,skype_apps_domain))
...
if __name__ == '__main__':
  ...
  all_skype_profiles = skype_adv_remover.find_all_skype_profiles_winreg()
  if not all_skype_profiles:
    all_skype_profiles =  skype_adv_remover.find_all_skype_profiles()

  skype_adv_remover.center_window(all_skype_profiles)
  skype_adv_remover.draw_profile_checkbuttons(all_skype_profiles)
